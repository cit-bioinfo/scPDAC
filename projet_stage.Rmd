...
#Creation of pool2 object with metadata
...
#load("~/Desktop/stage_rose/init.RData")
```
...
pool2[["percent.mt"]] <- PercentageFeatureSet(object = pool2, pattern = "^MT-")
...
pool
pool2 <- subset(pool2, subset = nFeature_RNA > 200 & percent.mt < 10)
pool2
```
...
pool.init <-  NormalizeData(pool.init, normalization.method = "LogNormalize", scale.factor=10000)
```
...
pool2 <- FindVariableFeatures(object=pool2, selection.method = "vst", nfeatures = 2000)
head(x = HVFInfo(object = pool))
head(x = HVFInfo(object = pool2))

#The 10 most higly variable genes
top10.variablegenes <- head(VariableFeatures(pool), 10)
top10.variablegenes2 <- head(VariableFeatures(pool2), 10)
top10.variablegenes
top10.variablegenes2
...
pool2 <- RunPCA(pool2, features = VariableFeatures(object = pool2))
...
pool2 <- FindNeighbors(pool2, reduction='pca', dims = 1:10)
...
pool2 <- FindClusters(pool2, resolution = 1)
```
...
pool2 <- RunUMAP(pool2, dims = 1:10)
```

### T-distributed stochastic neighbor embedding (tSNE)
```{r, results="hide"}
# pool2 <- RunTSNE(pool2, dims.use = 1:10)
# 
# plot8 <- DimPlot(object = pool2, reduction = "tsne", label=TRUE, repel=TRUE, group.by="orig.ident")
# plot9 <- DimPlot(object = pool2, reduction = "tsne", label=TRUE, repel=TRUE, group.by="cluster") + NoLegend()
# plot8 + plot9
```
...
cluster1_markers <- FindMarkers(pool2, ident.1 = 1)
...
feature <- c("MET")
#The MET gene is present in the ductall cells 2 
```
...
### **Clustering tools - En utilisant pool.init**

#### **BuildClusterTree**

```{r}
nbt = BuildClusterTree(pool.init)
Tool(object = nbt, slot = 'BuildClusterTree')
PlotClusterTree(object = nbt)
```
...
plot6 <- DimPlot(pool2, reduction = "umap", pt.size = 0.5, cols=c("#99CCFF", "coral"))
plot6 
...
###MCPcounter
mcp <- readRDS("~/Université/BIOLOGIE/Master Bioinformatique/M1 Bioinformatique/Semestre 2/Stage M1/Projet/data/supplementary_information/mcpgenes.rda")
head(mcp)
```
