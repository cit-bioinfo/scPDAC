...
pool_markers_test_avg <- subset(pool_markers_test, avg_logFC >=1)
head(pool_markers)
...
table_1_50 <- table(pool.sce$seurat_clusters, celltypes(fit_1_50))
table_1_50
table_25_75 <- table(pool.sce$seurat_clusters, celltypes(fit_25_75))
table_25_75
table_50_100 <- table(pool.sce$seurat_clusters, celltypes(fit_50_100))
table_50_100

print(table(pool.sce$seurat_clusters, celltypes(fit_1_50)))
...
library(dplyr)
library(matrixStats)
#Treatment of the normal and tumor samples from inferCNV 
...
sum(infer_CNV_T_colMeans >=quantile_N)
#4161 : CNA +
sum(infer_CNV_T_colMeans < quantile_N)
#9041 : CNA -
```
