...
pool.init <- FindClusters(pool.init, resolution = 0.8)
...
counts2 <- data.matrix(counts, rownames.force = NA) #matrix
metadata2 <- data.matrix(metadata, rownames.force = NA) #cell_type 
sce <- SingleCellExperiment(list(counts=counts2), colData=DataFrame(metadata2))
print(sce)

#Find all markers for pool2
Idents(pool2) <- "cluster"
pool2_markers <- FindAllMarkers(pool2, only.pos = TRUE)
#write_xlsx(pool2_markers, "~/Documents/projet_stage/data/pool2_markers.xlsx")
load("/home/rose/Documents/projet_stage/data/pool2_markers.RData")

pool2_markers <- pool2_markers[6:7]
...
split_pool2_markers <- split(pool2_markers[2], pool2_markers[[1]])
str(head(split_pool2_markers))

#unlist_pool2_markers <- unlist(split_pool2_markers)
#unlist_pool2_markers <- unlist(split_pool2_markers, use.names=FALSE)
#head(unlist_pool2_markers)

list_pool2_markers <- lapply(split_pool2_markers, function(x) unlist(x, use.names=FALSE))
str(head(list_pool2_markers))  

#Creation of the marker gene matrix
mat_pool2_markers <- marker_list_to_mat(list_pool2_markers, include_other = FALSE)
mat_pool2_markers[1:10,]

#frame_mat_pool2_markers <- as.data.frame(mat_pool2_markers)
#write_xlsx(frame_mat_pool2_markers, "~/Documents/projet_stage/data/mat_pool2_markers.xlsx")
```
...
fit <- cellassign(exprs_obj=sce[rownames(mat_pool2_markers),], marker_gene_info=mat_pool2_markers, s=s, learning_rate = 1e-2, shrinkage=TRUE, verbose=FALSE) #error, session avorted
...
###### with 1000 cells
```{r}
## Create Seurat object by counts matrix 
# sce2 <- CreateSeuratObject(counts, min.cells = 100)
# sce2 <- as.SingleCellExperiment(sce2)
# sce2 <- SingleCellExperiment(colData=DataFrame(metadata2))
# sce2
```
...
plot6 <- DimPlot(pool2, reduction = "umap", pt.size = 0.5, cols=c("lightblue", "lavender"))
plot6 
...
```{r}
#counts matrix
load("~/Documents/projet_stage/data/Peng_Data.RData")
...
infercnv_objt <- CreateInfercnvObject(counts, annotations_file="~/Documents/projet_stage/data/inferCNV/metadata.txt", gene_order_file ="~/Documents/projet_stage/data/inferCNV/gen_order.txt", ref_group_names=NULL)
...
write.table(pool@meta.data, file="~/Documents/projet_stage/data/analysis/metadata_pool.csv", sep="\t", row.names=T, col.names=T, quote=F)

write.table(pool@active.ident, file="~/Documents/projet_stage/data/analysis/active_ident_pool.tsv", sep="\t", row.names=T, col.names=T, quote=F)
```

#### POOL2
```{r}
write.table(pool2@meta.data, file="~/Documents/projet_stage/data/analysis/metadata_pool2.tsv", sep="\t", row.names=T, col.names=T, quote=F)

write.table(pool2@meta.data, file="~/Documents/projet_stage/data/analysis/active_ident_pool2.tsv", sep="\t", row.names=T, col.names=T, quote=F)
```
